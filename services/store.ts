...
const initialPointMovements: PointMovement[] = [
    { id: 'pm1', clientId: 'c1', orderId: 'o2', type: 'acumulacion', points: 250, timestamp: new Date().toISOString() },
];

const initialExpenseCategories: ExpenseCategory[] = ["Insumos", "Alquiler", "Sueldos", "Mantenimiento", "Servicios Públicos", "Marketing", "Otros"];
...
const createNotificationLog = (clientId: string, template: string, variables: Record<string, any>): NotificationLog => {
...
const store = {
    ...
    getCalendarDataForMonth: (year: number, month: number): CalendarData => {
    ...
    getOrdersByDeliveryDate: (date: string): Order[] => {
        return state.orders.filter((order: Order) => order.deliveryDate === date);
    },
    ...
    logout: () => {
        ...
        if (currentUser) {
            ...
            if (lastRecord && lastRecord.checkOut === null) {
                ...
                const newAttendance = attendance.map((a: AttendanceRecord) => a.id === updatedRecord.id ? updatedRecord : a);
    ...
    moveOrder: (orderId: string, newStatus: OrderStatus) => {
        const originalOrder = state.orders.find((o: Order) => o.id === orderId);
        ...
        if (prevStageChecklist) {
            prevStageChecklist.forEach((task: ChecklistItem) => {
    ...
    updateOrder: (updatedOrder: Order) => {
        const originalOrder = state.orders.find((o: Order) => o.id === updatedOrder.id);
        if (!originalOrder) return;
        ...
        if (originalOrder.assignedTo !== updatedOrder.assignedTo) {
            const oldE = state.employees.find((e: Employee) => e.id === originalOrder.assignedTo)?.name || 'Nadie';
            const newE = state.employees.find((e: Employee) => e.id === updatedOrder.assignedTo)?.name || 'Nadie';
            newHistory.push(createLogEntry(`Reasignado de ${oldE} a ${newE}.`));
        }

        // Machine Change
        if (originalOrder.machineId !== updatedOrder.machineId) {
            const oldM = state.equipment.find((m: Equipment) => m.id === originalOrder.machineId)?.name || 'Ninguna';
            const newM = state.equipment.find((m: Equipment) => m.id === updatedOrder.machineId)?.name || 'Ninguna';
        ...
        if(originalTasks.length < updatedTasks.length) {
            const newTasks = updatedTasks.filter((t: string) => !originalTasks.includes(t));
            newTasks.forEach((taskId: string) => {
                const taskLabel = CHECKLISTS[originalOrder.status]?.find((t: ChecklistItem) => t.id === taskId)?.label || taskId;
                newHistory.push(createLogEntry(`Tarea completada: "${taskLabel}".`));
            });
        }

        const finalOrder = { ...updatedOrder, history: newHistory };
        const orders = state.orders.map((o: Order) => (o.id === finalOrder.id ? finalOrder : o));
        store.setState({ orders });
    },
    completeTask: (orderId: string, taskId: string) => {
        ...
        const taskDefinition = stageChecklist.find((t: ChecklistItem) => t.id === taskId);
    ...
    addOrUpdateInventoryItem: (item: InventoryItem) => {
        const index = state.inventory.findIndex((i: InventoryItem) => i.id === item.id);
        if (index > -1) {
    ...
    deleteService: (serviceId: string) => {
        const services = state.services.map((s: Service) => {
            if (s.id === serviceId) {
    ...
    addOrUpdateEmployee: (employeeData: Employee): Employee | undefined => {
        const index = state.employees.findIndex((e: Employee) => e.id === employeeData.id);
        let finalEmployeeData: Employee | undefined = undefined;
    ...
    addOrUpdateEquipment: (equipmentItem: Equipment) => {
        const index = state.equipment.findIndex((e: Equipment) => e.id === equipmentItem.id);
        if (index > -1) {
    ...
    deleteEquipment: (equipmentId: string) => {
        const equipment = state.equipment.filter((e: Equipment) => e.id !== equipmentId);
        store.setState({ equipment });
    },
    toggleEquipmentStatus: (equipmentId: string) => {
        const equipment = state.equipment.map((e: Equipment) => {
            if (e.id === equipmentId) {
                return { ...e, status: e.status === 'Operativa' ? 'Fuera de Servicio' : 'Operativa' } as Equipment;
            }
            return e;
        });
        store.setState({ equipment });
    },
    addOrder: (order: Omit<Order, 'id' | 'numericId' | 'history' | 'usedSupplies' | 'garments' | 'info_empaque' | 'completedTasks'>): Order => {
        const { orders } = store.getState();
        const maxId = Math.max(...orders.map((o: Order) => o.numericId), 0);
    ...
    setTurneroFilter: (filter: Partial<TurneroFilter>) => {
        const newFilters = { 
            ...state.filters,
    ...
    setInventoryFilter: (filter: Partial<InventoryFilter>) => {
        const newFilters = { 
            ...state.filters,
    ...
    // Marketing Actions
    getApplicablePromotions: (serviceIds: string[]): Promotion[] => {
        const now = new Date();
        return state.promotions.filter((p: Promotion) => {
            const isDateValid = new Date(p.startDate) <= now && new Date(p.endDate) >= now;
            const appliesToServices = p.applicableServices.length === 0 || p.applicableServices.some((sId: string) => serviceIds.includes(sId));
            return p.active && isDateValid && appliesToServices;
        });
    },

    addOrUpdatePromotion: (promo: Promotion) => {
        const index = state.promotions.findIndex((p: Promotion) => p.id === promo.id);
        if (index > -1) {
    ...
    deletePromotion: (promoId: string) => {
        const promotions = state.promotions.filter((p: Promotion) => p.id !== promoId);
        store.setState({ promotions });
    },

    updateLoyaltyConfig: (config: LoyaltyConfig) => {
    ...
    addExpenseCategory: (category: string) => {
        ...
        if (normalizedCategory && !expenseCategories.some((c: string) => c.toLowerCase() === normalizedCategory.toLowerCase())) {
            store.setState({ expenseCategories: [...expenseCategories, normalizedCategory].sort() });
        }
    },

    processPayment: (details: { orderId: string, amountPaid: number, method: PaymentMethod, isAdvance: boolean, pointsToRedeem: number }): boolean => {
        ...
        let client = clients.find((c: Client) => c.id === order.clientId);
        ...
        const updatedClients = clients.map((c: Client) => c.id === client!.id ? client! : c);
    ...
    addExpense: (expense: Omit<Expense, 'id'>) => {
        ...
        const newExpense: Expense = { ...expense, id: `ex${Date.now()}` };
        
        store.addExpenseCategory(newExpense.category);
        
        const newGeneratedPayables: AccountPayable[] = [];
    ...
    markAccountPayableAsPaid: (accountId: string) => {
        let { accountsPayable } = store.getState();
        const updatedAccountsPayable = accountsPayable.map((acc: AccountPayable) => {
            if (acc.id === accountId) {
                return {
                    ...acc,
    ...
    sendMarketingNotification: (clientIds: string[], message: string) => {
    ...
    // Reports Actions
    generateExpenseByCategory: (startDate: string, endDate: string): ExpenseByCategory[] => {
        ...
        state.accountsPayable.forEach((ap: AccountPayable) => {
            if (ap.status === 'pagada' && ap.paymentDate) {
                const paymentDate = new Date(ap.paymentDate + 'T00:00:00');
                if (paymentDate >= start && paymentDate <= end) {
                    const originalExpense = state.expenses.find((e: Expense) => e.id === ap.expenseId);
                    if (originalExpense) {
        ...
        return Object.entries(results).map(([category, amount]: [string, number]) => ({
            category: category as ExpenseCategory,
            amount,
        })).sort((a: ExpenseByCategory, b: ExpenseByCategory) => b.amount - a.amount);
    },

    generateEmployeePerformance: (startDate: string, endDate: string): EmployeePerformance[] => {
        const start = new Date(startDate + 'T00:00:00');
        const end = new Date(endDate + 'T23:59:59');
        const performance: Record<string, { employeeName: string, tasksCompleted: number }> = {};

        state.orders.forEach((order: Order) => {
            order.history.forEach((log: LogEntry) => {
                const logDate = new Date(log.timestamp);
                if (logDate >= start && logDate <= end && log.action.startsWith('Tarea completada:')) {
                    const employee = state.employees.find((e: Employee) => e.id === log.userId);
                    if (employee) {
        ...
        return Object.entries(performance).map(([employeeId, data]) => ({
            employeeId,
            ...data
        })).sort((a: EmployeePerformance,b: EmployeePerformance) => b.tasksCompleted - a.tasksCompleted);
    },
    
    generateSupplyUsage: (startDate: string, endDate: string): SupplyUsage[] => {
        const start = new Date(startDate + 'T00:00:00');
        const end = new Date(endDate + 'T23:59:59');
        const usage: Record<string, { supplyName: string, quantityUsed: number, unit: 'L' | 'Kg' | 'Uds' }> = {};

        state.orders.forEach((order: Order) => {
            if (order.status === OrderStatus.Delivered) {
                const receptionDate = new Date(order.receptionDate + 'T00:00:00');
                if (receptionDate >= start && receptionDate <= end) {
                    order.usedSupplies.forEach((used: UsedSupply) => {
                        const supplyInfo = state.inventory.find((i: InventoryItem) => i.id === used.inventoryItemId);
                        if (supplyInfo) {
        ...
        return Object.entries(usage).map(([supplyId, data]) => ({
            supplyId,
            ...data
        })).sort((a: SupplyUsage, b: SupplyUsage) => b.quantityUsed - a.quantityUsed);
    },

    generateTopClients: (startDate: string, endDate: string): TopClient[] => {
        const start = new Date(startDate + 'T00:00:00');
        const end = new Date(endDate + 'T23:59:59');
        const clientData: Record<string, { clientName: string; orderCount: number; totalSpent: number; }> = {};
        
        state.orders.forEach((order: Order) => {
            const receptionDate = new Date(order.receptionDate + 'T00:00:00');
            if (receptionDate >= start && receptionDate <= end) {
                const clientInfo = state.clients.find((c: Client) => c.id === order.clientId);
                if (clientInfo) {
    ...
    generateKpiReport: (startDate: string, endDate: string): KpiReportData => {
        const start = new Date(startDate + 'T00:00:00');
        const end = new Date(endDate + 'T23:59:59');

        const deliveredOrdersInPeriod: {order: Order, deliveryTimestamp: string}[] = [];

        state.orders.forEach((order: Order) => {
            if (order.status === OrderStatus.Delivered) {
                const finalPayment = state.payments
                    .filter((p: Payment) => p.orderId === order.id && !p.isAdvance)
                    .sort((a: Payment,b: Payment) => new Date(b.paymentDate).getTime() - new Date(a.paymentDate).getTime())[0];
        ...
        if (deliveredOrdersInPeriod.length > 0) {
            deliveredOrdersInPeriod.forEach(({ order, deliveryTimestamp }: {order: Order, deliveryTimestamp: string}) => {
        ...
        const cycleTimes: KpiCycleTime[] = Object.entries(serviceCycleTimes).map(([serviceId, durations]: [string, number[]]) => {
            const service = state.services.find((s: Service) => s.id === serviceId);
            const avgHours = durations.reduce((sum: number, d: number) => sum + d, 0) / durations.length;
            return {
        ...
        // 3. Loyalty
        const ordersForLoyalty = state.orders.filter((order: Order) => {
            const receptionDate = new Date(order.receptionDate + 'T00:00:00');
            return receptionDate >= start && receptionDate <= end;
        });

        const clientOrderCounts: Record<string, number> = {};
        ordersForLoyalty.forEach((order: Order) => {
        ...
        let loyalty: KpiLoyalty = { recurrentCustomerRate: 0, avgOrdersPerCustomer: 0 };

        if (uniqueClients > 0) {
            const recurrentClients = Object.values(clientOrderCounts).filter((count: number) => count > 1).length;
